...
uint8_t ST25R3916B_14A4_Rats(ST25R *pInstance, const uint8_t parameters, T4A_INFOS *pt4aInfos)
{
	uint8_t ret, buffer[2] = {K14A_RATS, parameters};

	ret = ST25R3916B_Transmit_then_Receive(pInstance, buffer, sizeof(buffer), 1);
	if(ret == ST25R_STATUS_NO_ERROR)
	{
		if (pInstance->cbData <= sizeof(pt4aInfos->ATS))
		{
			...
			ST25R3916B_Write_SingleRegister(pInstance, ST25R3916B_REG_TIMER_EMV_CONTROL, ST25R3916B_REG_TIMER_EMV_CONTROL_nrt_step_4096_fc);
			ST25R3916B_Write_NoResponseTimer(pInstance, 1 << pt4aInfos->FWI);
...
uint8_t ST25R3916B_14A4_Deselect(ST25R *pInstance)
{
...
uint8_t ST25R3916B_14A4_AdjustBitRate(ST25R *pInstance, T4A_INFOS *pt4aInfos, ST25R_BITRATE bitrate)
{
	...
	if(pt4aInfos->CurrentBitrate != bitrate)
	{
		...
		if(ret == ST25R_STATUS_NO_ERROR)
		{
			...
			case ST25R_BITRATE_848:
				ST25R3916B_14A4_TxRx848(pInstance);
				break;

			case ST25R_BITRATE_424:
				ST25R3916B_14A4_TxRx424(pInstance);
				break;

			case ST25R_BITRATE_212:
				ST25R3916B_14A4_TxRx212(pInstance);
				break;
