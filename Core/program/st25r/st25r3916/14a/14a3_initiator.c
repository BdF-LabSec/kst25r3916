...
uint8_t K14A3_Anticoll_CL(ST25R *pInstance, uint8_t cl, uint8_t UID[10], uint8_t *pSAK, uint8_t *pUIDIdx)
{
...
uint8_t K14A3_Anticoll(ST25R *pInstance, T3A_INFOS *pInfos)//uint16_t *pATQA, uint8_t *pUID, uint8_t *pcbUID, uint8_t *pSAK)
{
	uint8_t ret;
	...
	ST25R3916_Write_SingleRegister(pInstance, ST25R3916_REG_AUX, ST25R3916_REG_AUX_no_crc_rx);
	ST25R3916_Write_NoResponseTimer(pInstance, 0x0023);
	...
	if(ret == ST25R_STATUS_NO_ERROR)
	{
		if(pInstance->cbData == sizeof(pInfos->ATQA))
		{
			...
			{
				ret = K14A3_Anticoll_CL(pInstance, K14A_SDD_REQ_CL1, pInfos->UID, &pInfos->SAK, &pInfos->cbUID);
				if ((ret == ST25R_STATUS_NO_ERROR) && (pInfos->SAK & 0x04))
				{
					ret = K14A3_Anticoll_CL(pInstance, K14A_SDD_REQ_CL2, pInfos->UID, &pInfos->SAK, &pInfos->cbUID);
					if ((ret == ST25R_STATUS_NO_ERROR) && (pInfos->SAK & 0x04))
					{
						ret = K14A3_Anticoll_CL(pInstance, K14A_SDD_REQ_CL3, pInfos->UID, &pInfos->SAK, &pInfos->cbUID);
						if ((ret == ST25R_STATUS_NO_ERROR) && (pInfos->SAK & 0x04))
						{
	...
	if(ret == ST25R_STATUS_NO_ERROR)
	{
		ST25R3916_Write_NoResponseTimer(pInstance, 0x0423); // TODO
	}

	return ret;
}

const uint8_t K14A3_HLTA_data[] = {K14A_HLTA, 0x00};
void K14A3_HLTA(ST25R *pInstance)
{
	ST25R3916_Transmit(pInstance, K14A3_HLTA_data, sizeof(K14A3_HLTA_data), 1);
	HAL_Delay(1);
}
